...
import { readDb, writeDb, Card, CardTier, addXp, emojiTiers, calculateLevel } from "@/lib/db";
...
export async function POST(req: NextRequest) {
  ...
  const newCard: Card = {
    ...
    holder: body.holder || "CARD HOLDER",
  ...
  if (body.sourceCardId && body.upgradeCost) {
    const sourceIdx = db.cards.findIndex((c) => c.id === body.sourceCardId);
    if (sourceIdx !== -1) db.cards[sourceIdx].balance -= body.upgradeCost;
  }
  const isFirstCard = db.cards.length === 0;
  db.cards.push(newCard);
  const levelUps = addXp(db, isFirstCard ? 5 : 3);
  writeDb(db);
  const { level, currentXp, nextXp } = calculateLevel(db.user.xp);
  return NextResponse.json({ card: newCard, levelUps, level, currentXp, nextXp, xp: db.user.xp }, { status: 201 });
}
...
export async function PATCH(req: NextRequest) {
  const db = ensureBaseData();
  const body = await req.json();
  const idx = db.cards.findIndex((c) => c.id === body.id);
