...
export default function TttPage() {
  ...
  const makeMove = async (index: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const res2 = await fetch("/api/games/ttt/sync", { method: "POST", body: JSON.stringify({ token: gameToken }) }).catch(() => null);
        if (res2?.ok) { const d = await res2.json(); setBoard(d.board); }
      }
    } catch {
      toast.error("Сетевая ошибка");
    } finally {
  ...
  return (
    ...
    >
      {gameState === "lobby" ? (
        <div className="max-w-sm mx-auto w-full space-y-4 mt-4">
          ...
          <Button onClick={startGame} disabled={actionLoading} className="w-full h-11 font-semibold" variant="gradient">
            {actionLoading ? <RiRefreshLine className="animate-spin w-4 h-4" /> : "Начать матч"}
          </Button>
        </div>
      ) : (
        <div className="flex flex-col items-center space-y-8">
          ...
          <div className="h-12 flex flex-col items-center justify-center">
            {gameState === "playing" ? (
              <p className="text-sm text-muted-foreground font-medium">Ваш ход (X)</p>
            ) : (
