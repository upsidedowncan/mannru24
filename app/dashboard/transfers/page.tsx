  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <CreateCardDialog onCreated={() => fetch("/api/cards").then(r => r.json()).then(setCards)} existingCards={cards.map(c => ({ id: c.id, tier: c.tier, balance: c.balance, label: `${tierMeta[c.tier].label} ••${c.number.slice(-4)}` }))} disabled={isReadOnly} />
      </div>
